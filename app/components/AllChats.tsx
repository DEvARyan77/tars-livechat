...
export default function AllChats({
  ...
  const [menuPosition, setMenuPosition] = useState<{ top: number; left: number; isMe: boolean } | null>(null);
  const menuRef = useRef<HTMLDivElement>(null);

  const activeMessage = messages?.find(m => m._id === activeEmojiMenu);
  const isActiveMessageMe = activeMessage?.senderId === currentUser._id;

  useEffect(() => {
    const container = scrollContainerRef.current;
    if (!container || !activeEmojiMenu) return;
    const onScroll = () => setActiveEmojiMenu(null);
    container.addEventListener("scroll", onScroll);
    return () => container.removeEventListener("scroll", onScroll);
  }, [activeEmojiMenu, scrollContainerRef, setActiveEmojiMenu]);
  ...
  useEffect(() => {
    ...
    const messageEl = document.getElementById(`message-${activeEmojiMenu}`);
    if (!messageEl) return;

    const rect = messageEl.getBoundingClientRect();
    const isMe = messageEl.classList.contains("justify-end");

    setMenuPosition({
      top: rect.top + window.scrollY - 60,
      left: isMe ? rect.right - 200 : rect.left,
      isMe,
    });
  }, [activeEmojiMenu]);
  ...
  return (
    <>
              ...
              >
                <div className="max-w-[75%] group relative">
                  ...
                  {(isMe || msg.deleted) && (
                    <button
                      ...
                      className={`absolute -top-2 ${
                        isMe ? "-left-2" : "-right-2"
                      } bg-white border-2 border-black p-1 rounded-full z-20 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-400 active:translate-y-[1px] shadow-[2px_2px_0px_rgba(0,0,0,1)]`}
                    >
                      {msg.deleted ? (
                  ...
                  <div
                    ...
                    ${
                      isMe
                        ? "bg-[#ffcf00] text-black rounded-b-2xl rounded-tl-2xl"
                  ...
                  {msg.reactions && msg.reactions.length > 0 && (
                    ...
                    >
                      <button
                        ...
                        className={`flex items-center h-fit gap-1 bg-white border-2 border-black pl-1/2 py-0.5 rounded-full active:translate-x-[1px] active:translate-y-[1px] active:shadow-none transition-all ${
                      ...
                      >
                        <div className="flex -space-x-1 items-center">
                          ...
                          {msg.reactions.length > 3 && (
                            <span className="text-[11px] mr-1 font-black font-sans ml-1 text-black/80">
                              +{msg.reactions.length - 3}
                            </span>
                          )}
                        </div>
                  ...
                  <div
                    className={`text-[10px] mt-2 font-black text-zinc-400 ${
      ...
      {activeEmojiMenu && menuPosition && activeMessage && !isActiveMessageMe &&
        createPortal(
          <div
            ref={menuRef}
            style={{
          ...
          >
            <div
              className="flex gap-2 bg-white border-2 border-black p-1.5 rounded-xl shadow-[3px_3px_0px_rgba(0,0,0,1)] justify-center items-center px-0"
            >
              {EMOJIS.map((emoji) => (
                <button
                  key={emoji}
                  onClick={() => handleReaction(activeEmojiMenu, emoji)}
              ...
              {showFullPicker && (
                <div className="absolute top-full left-0 mt-2 z-[99999] origin-top-left transition-all">
                  <div className="border-4 border-black rounded-2xl overflow-hidden shadow-[8px_8px_0px_rgba(0,0,0,1)] bg-white">
                    <EmojiPicker
                      onEmojiClick={(e) => handleReaction(activeEmojiMenu, e.emoji)}
                      width={320}
                      height={300}
                      theme={Theme.LIGHT}
